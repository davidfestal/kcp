	...
	if intendedToBeRemovedFromLocation && !stillOwnedByExternalActorForLocation {
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				...
				if err := shared.EnsureUpstreamFinalizerRemoved(ctx, gvr, upstreamSyncerLister, c.upstreamClient, upstreamObj.GetNamespace(), c.syncTargetKey, upstreamObjLogicalCluster, upstreamObj.GetName()); err != nil {
					return err
				}
				return nil
			}
