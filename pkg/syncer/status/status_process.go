...
func (c *Controller) process(ctx context.Context, gvr schema.GroupVersionResource, key string) error {
	...
	if !resourceExists {
		...
		return shared.EnsureUpstreamFinalizerRemoved(ctx, gvr, upstreamLister, c.upstreamClient, upstreamNamespace, c.syncTargetKey, upstreamWorkspace, shared.GetUpstreamResourceName(gvr, downstreamName))
	}
	...
	return c.updateStatusInUpstream(ctx, gvr, upstreamLister, upstreamNamespace, upstreamName, upstreamWorkspace, u)
}

func (c *Controller) updateStatusInUpstream(ctx context.Context, gvr schema.GroupVersionResource, upstreamLister kcpcache.GenericClusterLister, upstreamNamespace, upstreamName string, upstreamLogicalCluster logicalcluster.Name, downstreamObj *unstructured.Unstructured) error {
	logger := klog.FromContext(ctx)
	...
	existingObj, err := upstreamLister.ByCluster(upstreamLogicalCluster).ByNamespace(upstreamNamespace).Get(upstreamName)
	if err != nil {
