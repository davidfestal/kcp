...
func (c *PersistentVolumeClaimController) process(ctx context.Context, key string) error {
	...
	case corev1.ClaimBound:
		...
		nsLocatorJSONToByte := []byte(nsLocatorJSON)
		var p map[string]interface{}

		err = json.Unmarshal(nsLocatorJSONToByte, &p)
		if err != nil {
			return fmt.Errorf("failed to unmarshal blob %q: %w", nsLocatorJSON, err)
		}
		delete(p, "namespace")

		nsLocatorJSONToByte, err = json.Marshal(p)
		if err != nil {
