...
func (c *Controller) GVRs() map[schema.GroupVersionResource]informer.GVRPartialMetadata {
	...
	gvrs := make(map[schema.GroupVersionResource]informer.GVRPartialMetadata, len(c.gvrs))
...
func (c *Controller) Synced() bool {
	// TODO(davidfestal): Report the Synced() function of the Synctarget informer here ?
	return true
}
...
func (s *Controller) FilterBuiltinGVRs(gvr schema.GroupVersionResource) bool {
	if shared.IsNamespace(gvr) {
		return true
	}
	return builtinGVRs[gvr]
}
...
var builtinGVRs = map[schema.GroupVersionResource]bool{
	{
		Version:  "v1",
		Resource: "configmaps",
	}: true,
